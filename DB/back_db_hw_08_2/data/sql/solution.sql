...
SELECT user.id, board.title from user JOIN board ON user.name = board.writername where board.title LIKE '%헐크%';

#4. 2018년 이전에 작성된 board를 작성한 user의 email과 createtime을 출력하시오.
select user.email, board.createtime from user LEFT JOIN board ON user.name = board.writername where YEAR(board.createtime) < '2018' ORDER BY board.createtime DESC;
...
select board.title from (SELECT age, name FROM user WHERE age between 30 AND 60) temp LEFT JOIN board ON temp.name = board.writername ORDER BY name DESC;
